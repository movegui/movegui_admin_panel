import 'package:flutter/material.dart';

class AppColors {
  /*
  static const Color lightScaffoldColor = Colors.white;
  static const Color lightPrimary = Color(0xFF181059);
  static const Color lightCardColor = Color.fromARGB(106, 250, 250, 250);
  static const Color darkScaffoldColor = Color.fromARGB(255, 9, 3, 27);
  static const Color darkPrimary = Color.fromRGBO(94, 75, 236, 1);
  static const Color backgroundColor = Color(0xFF871A1C);
  */
  static const Color selectionColor = Colors.yellow;
  static const Color error = Colors.red;
  static const Color onPrimary = Color(0xFFFFFFFF);
  static const Color placeHolderText = Colors.grey;

  //********************************************** */

  static const Color primary = Color(0xFF90171B);
  static const Color lightBackground = Color.fromARGB(255, 114, 78, 223);
    static const Color darkBackground = Color.fromARGB(255, 33, 33, 34);
  static const Color card = Color(0xFFF1FFFF);
  static const Color success = Color(0xFF4CAF50);
  static const Color warning = Color(0xFFFFA000);
  static const Color info = Color(0xFF2196F3);
  static const Color on_delivery =  Color(0xFF7B1FA2);
  static const border = Color(0xFFE0E0E0);
  static const textPrimary = Color(0xFF212121);
  static const textSecondary = Color(0xFF757575);
}
